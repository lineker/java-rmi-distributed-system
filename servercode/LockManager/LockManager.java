...
public class LockManager
{
                ...
                synchronized (this.lockTable) {
                    ...
                    if (!bConflict) { //if false
                        ...
                        if (bConvert.get(0) == true) {
                        	...
                        	//find trxnObj on lockTable
                        	TrxnObj t = (TrxnObj)this.lockTable.get(new TrxnObj(xid,strData,TrxnObj.READ));
                        	//change trxnObj LockType
                        	t.setLockType(WRITE);
                        	
                        	//find dataObj on lockTable
                        	DataObj d = (DataObj)this.lockTable.get(new DataObj(xid,strData,0)); // 0 = READ
                        	//change dataObj LockType
                        	d.setLockType(WRITE);
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (dataObj.getXId() == dataObj2.getXId()) {    
                ...
                if (dataObj.getLockType() == DataObj.READ) {    
                	...
                	// we just have to - throw new RedundantLockRequestException(dataObj.getXId(), "Redundant READ lock request");
                	...
                	{
                		// we have to convert to WRITE
                		if(size == 1)
                		{
                			//dataObj2.setLockType(WRITE);
                			bitset.set(0, true);
                			return false;
                		}
                		else
    ...
    public void printLockTable()
    {
    	...
    	Vector vect = this.lockTable.allElements();
    	for (int i = 0; i < vect.size(); i++) {
    		if(vect.elementAt(i).getClass() == TrxnObj.class)
    		{
    			TrxnObj dataObj2 = (TrxnObj) vect.elementAt(i);
    			System.out.println("TID: "+dataObj2.getXId()+" DATA:"+dataObj2.getDataName()+" LOCK:"+getLockName(dataObj2.getLockType()));
    		}
    		else
    		{
    ...
    private String getLockName(int i)
    {
    	if(i == 0)
    		return "READ";
    	else
