...
public class client
{
    ...
    public static void main(String args[])
	{
	    ...
	    String server = "localhost"; //address of middleware
	    int port = 3000; //port of registry middleware
	    if(args.length > 1)
	    {
	    	server = args[0];
		...
		{
			System.out.println ("Usage: java client [ip] [port] ['tcp']"); 
			System.exit(1); 
		}
	    ...
	    else  if(args.length == 3)
	    {
	    	...
	    	{
				System.out.println("inside tcp instance");
	    		//rm = (ResourceManager) new ResourceClientTcp(1,server,port);
	    		istcp = true;
	    	}
	    	else if(args[2].equalsIgnoreCase("ws"))
	    	{
	    		//instantiate
	    		System.out.println("inside WS instance");
	    		rm = (ResourceManager) new ResourceClientWS(server,port);
	    		isws = true;
	    	}
	    ...
	    if(args.length > 3)
	    {
	    	...
	    	mID  = Integer.parseInt(args[2]);
			warmup = Boolean.parseBoolean(args[3]);
			int numberOfOp = Integer.parseInt(args[4]);
			System.out.println("starting automated client : "+mID);
			try {
				int tid;
				tid = rm.start();
				System.out.println("TID : "+tid+" automated client : "+mID);
				//rm.newCustomer(tid);
				if(warmup)
				{
			    	warmup(tid);
...
//			/boolean customer=rm.newCustomerWithId(Id,Cid);
		...
		case 23:
			try {
				rm.dump(obj.getString(arguments.elementAt(1)));
			} catch (RemoteException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} catch (Exception e) {
		...
		case 25:
			...
			try {
				int TID = Integer.parseInt((String)arguments.elementAt(1));
				rm.commit(TID);
			} catch (RemoteException  e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
		...
		case 27:
			...
			try {
				rm.addFlight(TID,1,50,10);
				rm.addCars(TID, "montreal", 50, 10);
				rm.addRooms(TID, "montreal", 50, 10);
		...
		case 28: 
			try {
				if( rm.shutdown() )
					...
					System.out.println("Can't shutdown because there are transactions running!");
	...
	if (argument.compareToIgnoreCase("help")==0)
	    ...
	    return 26;
	else if (argument.compareToIgnoreCase("populate")==0)
	    return 27;
	...
	switch(findChoice(command))
		...
		System.out.println("\nUsage:");
		System.out.println("\titinerary,<id>,<customerid>,<flightnumber1>....<flightnumberN>,<LocationToBookCarsOrRooms>,<Cars?>,<Room?>");
		break;
	    ...
	    case 23:  //dump
			System.out.println("Dump entries from RM");
			System.out.println("Purpose:");
			System.out.println("\tDebug");
			System.out.println("\nUsage:");
			System.out.println("\tdump, <RMname>");
			break;
    ...
    private static void warmup(int tid)
	{
		try 
		{
			for (int i = 1; i < 100; i++) {
				rm.addFlight(tid,Integer.parseInt(cmdSender.randInt(500)),50,10);
				
			}
			for (int i = 1; i < 100; i++) {
				rm.addCars(tid, cmdSender.pickLocation(), 10, 50);
			}
			
			for (int i = 1; i < 100; i++) {
				rm.addRooms(tid, cmdSender.pickLocation(), 10, 50);
			}
	...
	private static void runCmds(int tid, int numberOfOperations, client obj)
	{
		
		Long start = System.currentTimeMillis();
		for (int i = 0; i < numberOfOperations; i++) {
			
			cmdSender.sendCommand(obj, rm, tid);
		}
		Long end = System.currentTimeMillis();
		log("start : "+start+"\n end : "+end+"\n elapsed :"+ new Long(end-start).toString(), mID);
	}
