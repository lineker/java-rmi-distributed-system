...
public class RequestHandler extends Thread {
	/**
	 * The port to listen to. The default value is 2012.
	 */
	private static int mPort = 2012;
	
	private int middlePort = 3033;
	
	private String middleAddress = "localhost";
	...
	public void run(){
		...
		while(true){
			...
			System.out.println("Listening at port "+mPort+"...");
			try {
				...
				try {
					...
					else
					{
						...
						mThreads[freeThread] = new ResourceManagerImpl(mConnectionQueue.removeFirst(), 
								m_itemHT);
						((ResourceManagerImpl)mThreads[freeThread]).setMiddlewareInfo(middleAddress, middlePort);
